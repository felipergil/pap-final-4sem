...
public class OperacoesEJB {
    ...
    public Produto returnProduct(String query) {
        Produto produto = (Produto) em.createQuery("SELECT p from Produto p where p.simbolo = :simbolo").setParameter("simbolo", query).getSingleResult();
        return produto;
    }

    public List<Produto> retornaPorNome(String query) {
        List<Produto> produtos = em.createQuery("SELECT p from Produto p where p.titulo = :titulo").setParameter("titulo", query).getResultList();
        return produtos;
    }
    ...
    public List<Pedido> returnPedidoPorEmail(String query) {
        return em.createNamedQuery("Pedido.porEmail").setParameter("email", query).getResultList();
    }
