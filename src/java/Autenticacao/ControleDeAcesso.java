...
public class ControleDeAcesso implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response,
        ...
        }else if(session.getAttribute("USUARIOLogado") != null && (req.getRequestURI().endsWith("/LowPriceCell/finalizarcompra.xhtml"))) {
            redireciona("/LowPriceCell/payment.xhtml", response);  
      
        }else if(session.getAttribute("USUARIOLogado") != null && (req.getRequestURI().endsWith("/LowPriceCell/endereco.xhtml"))) {
            chain.doFilter(request, response);
        
        }else if(session.getAttribute("USUARIOLogado") != null && (req.getRequestURI().endsWith("/LowPriceCell/minhaconta.xhtml"))) {
            chain.doFilter(request, response);
                 
        }else if(session.getAttribute("USUARIOLogado") != null && (req.getRequestURI().endsWith("/LowPriceCell/payment.xhtml"))) {
            chain.doFilter(request, response);
        
            
        } else if ((session.getAttribute("USUARIOLogado") == null)
                && (!req.getRequestURI().endsWith("/LowPriceCell/index.xhtml"))
                && (!req.getRequestURI().endsWith("/LowPriceCell/minhaconta.xhtml"))
                ...
                && (!req.getRequestURI().endsWith("/LowPriceCell/minhaconta2.xhtml"))
